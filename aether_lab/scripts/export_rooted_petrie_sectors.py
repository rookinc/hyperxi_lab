...
OUTPUT = Path("cocycles/tables/rooted_petrie_parity_sector_table.json")
REPORT = Path("cocycles/tables/rooted_petrie_parity_sector_table_report.txt")
...
def main():
    ...
    for v in verts:
        chosen_cycles, even_edges, odd_edges = rooted_sector_for_vertex(v, adj, cycles, eps)
    ...
    report = []
    report.append("ROOTED LOCAL PETRIE PARITY SECTOR EXPORT")
    report.append("=" * 60)
    ...
    print(f"canonical petrie cycles available: {len(cycles)}")
    print(f"vertices: {len(out)}")
    print(f"chosen cycle counts per root: {chosen_counts}")
