...
loaded from artifacts/census/thalion_graph.g6.
"""
...
def load_canonical_g6() -> nx.Graph:
    if not GRAPH_PATH.exists():
        raise FileNotFoundError(f"Graph file not found: {GRAPH_PATH}")
    return nx.from_graph6_bytes(GRAPH_PATH.read_bytes().strip())
...
def export_mapping(mapping: Dict[Chamber, int], outpath: Path) -> None:
...
def main() -> None:
    ...
    canonical = load_canonical_g6()

    print("=" * 72)
    print("FLAG CANDIDATE C vs CANONICAL THALEAN GRAPH (.g6)")
    print("=" * 72)
    ...
    if iso:
        ...
        export_mapping(mapping, mapping_path)

        print()
        print("sample mapping entries:")
        for left, right in list(mapping.items())[:10]:
            print(f"  {left} -> {right}")
