...
def all_faces_of_dodecahedron(D):
    ...
    return sorted({cyc for cyc in found if len(cyc) == 5})
...
def main():
    print("=" * 80)
    print("PROBE SIGN ORBITS ON G15")
    print("=" * 80)
    ...
    # Canonical dodecahedral labeling of G15 fibers
    ...
    for x, y in G15.edges():
        ...
        if patt == ("L", "L"):
            sign[tuple(sorted((x, y)))] = "+"
        elif patt == ("N", "N"):
            sign[tuple(sorted((x, y)))] = "-"
        else:
            sign[tuple(sorted((x, y)))] = "?"

    print("sign histogram:", Counter(sign.values()))

    # Full automorphism group of G15
    gm = nx.algorithms.isomorphism.GraphMatcher(G15, G15)
    autos = list(gm.isomorphisms_iter())
    print("Aut(G15) size:", len(autos))

    # Keep only sign-preserving automorphisms
    sign_pres = []
    for phi in autos:
        ok = True
        for e, s in sign.items():
            u, v = e
            ee = tuple(sorted((phi[u], phi[v])))
            if sign[ee] != s:
                ok = False
                break
        if ok:
            sign_pres.append(phi)

    print("sign-preserving automorphisms:", len(sign_pres))

    # Edge orbits under sign-preserving group
    ...
    while unseen:
        ...
        while changed:
            changed = False
            for phi in sign_pres:
                imgs = {tuple(sorted((phi[u], phi[v]))) for (u, v) in orb}
                if not imgs.issubset(orb):
                    orb |= imgs
                    changed = True
        unseen -= orb
    ...
    print()
    print("EDGE ORBIT SIZES UNDER SIGN-PRESERVING GROUP")
    print("-" * 80)
    for i, orb in enumerate(sorted(orbits, key=len), start=1):
        labels = Counter(sign[e] for e in orb)
        print(f"orbit {i}: size={len(orb)} labels={dict(labels)}")
        print(" ", orb[:12], "..." if len(orb) > 12 else "")

    print()
    print("INTERPRETATION")
    print("-" * 80)
    print("If the orbits have sizes 16 and 14, then the sign split is exactly the")
    print("edge-orbit decomposition under the sign-preserving subgroup of Aut(G15).")
