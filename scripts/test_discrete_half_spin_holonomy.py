...
def main():
    print("=" * 80)
    print("DISCRETE HALF-SPIN HOLONOMY TEST")
    print("=" * 80)
    ...
    # Canonical labeling via dodecahedron
    ...
    for x, y in G15.edges():
        ...
        if patt == ("L", "L"):
            sign[tuple(sorted((x, y)))] = 0   # trivial sheet holonomy
        elif patt == ("N", "N"):
            sign[tuple(sorted((x, y)))] = 1   # nontrivial sheet holonomy
        else:
            raise RuntimeError(f"Unexpected pattern {patt} on edge {(x,y)}")

    print("edge sign histogram (+/0 trivial, -/1 twisted):", Counter(sign.values()))

    # test 5-cycles and 6-cycles in G15
    for k in (3, 4, 5, 6):
        cycles = simple_cycles_length_k(G15, k)
        hist = Counter()
        examples = []
        for cyc in cycles:
            edges = []
            ...
            hist[parity] += 1
            if len(examples) < 10:
                examples.append((cyc, parity, edges))

        print()
        print(f"CYCLE PARITY HISTOGRAM FOR LENGTH {k}")
        print("-" * 80)
        print(hist)
        print("sample cycles:")
        for cyc, parity, edges in examples:
            label = "odd/nontrivial" if parity else "even/trivial"
            print(f"  cycle {cyc} -> {label}")
            print(f"    edge signs: {edges}")

    print()
    print("INTERPRETATION")
    print("-" * 80)
    print("An odd cycle parity means one traversal of that loop swaps the two sheets")
    print("of the signed 2-lift G30 -> G15. Two traversals then return to the start.")
    print("That is the correct discrete 2π/4π analogue to look for.")
